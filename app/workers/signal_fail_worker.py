    ...
    with logger.contextualize(log_label="SIGNAL FAIL WORKER"):
        ...
        with redis_client_gateway.pipeline() as pipe:
            for key in tracker_keys:
                pipe.hmget(key, "is_hybrid", "hybrid_id", "last_packet_data", "last_merged_location", "protocol", "mode")
        ...
        for i, tracker_data in enumerate(all_trackers_data):
            ...
            protocol = tracker_data[4]
            mode = tracker_data[5]

            # Se não for um veículo híbrido, dropamos
            if not is_hybrid and not hybrid_id:
                if protocol == "satellital" and mode == "solo":
                    single_satellites_fail_worker(tracker_keys[i], tracker_data, failing_trackers, to_add_to_failing, to_remove_from_failing)
                
                continue
            else:
                hybrids_fail_worker(tracker_keys[i], tracker_data, failing_trackers, to_add_to_failing, to_remove_from_failing)
...
def single_satellites_fail_worker(key: str, tracker_data: tuple, failing_trackers: set, to_add_to_failing: set, to_remove_from_failing: set):
    last_merged_location_str = tracker_data[3]
    if not last_merged_location_str:
            logger.error(f"Pacote de dados inválidos! \n{last_merged_location_str}")
            return
    
    last_merged_location = json.loads(last_merged_location_str)
    satellite_last_timestamp = last_merged_location.get("timestamp")

    if satellite_last_timestamp:
...
def hybrids_fail_worker(key: str, tracker_data: tuple, failing_trackers: set, to_add_to_failing: set, to_remove_from_failing: set):
    try:
        ...
        if not last_gsm_location_str or not last_merged_location_str:
        ...
        if both_failed:
            packets_to_check = (last_merged_location,)
        else:
            packets_to_check = (last_gsm_location, last_merged_location)
        ...
        for packet in packets_to_check:
            if packet:
                try:
                    ...
                    logger.error(f"Error processing tracker data for key {key}: {e}")
