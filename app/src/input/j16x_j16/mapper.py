...
def decode_location_packet_4g(body: bytes):
    ...
    try:
        ...
        logger.exception(f"Falha ao decodificar pacote de localização 4G J16X-J16 body_hex={body.hex()}")
        return None
...
def handle_alarm_packet(dev_id_str: str, body: bytes):
    ...
    if universal_alert_id:
        logger.info(f"Alarme J16X-J16 (0x{alarm_code:02X}) traduzido para Universal ID {universal_alert_id} device_id={dev_id_str}")
        ...
        logger.warning(f"Alarme J16X-J16 não mapeado recebido device_id={dev_id_str}, alarm_code={hex(alarm_code)}")
