...
def build_generic_response(protocol_number: str, serial_number: int):
    """
    Constrói uma resposta genérica (ACK) para o dispositivo J16W-J16.
    """
...
def build_command(command_content_str: str, serial_number: int):
    """
    Cria comandos no padrão J16W-J16 para envio ao dispositivo
    """
    ...
    logger.info(f"Construído comando J16W-J16: {command_packet.hex()}")

    return command_packet

def process_command(dev_id: str, serial: int, universal_command: str):
    logger.info(f"Iniciando tradução de comando Universal para J16W-J16 device_id={dev_id}, comando={universal_command}")
    ...
    if not j16x_text_command:
        logger.warning(f"Nenhum mapeamento J16W-J16 encontrado para o comando Universal comando={universal_command}")
        return
    ...
    if tracker_socket:
        try:
            ...
            logger.info(f"Comando J16W-J16 enviado com sucesso device_id={dev_id}, comando_hex={j16x_binary_command.hex()}")
        except Exception:
            logger.exception(f"Falha ao enviar comando para o rastreador J16W-J16 device_id={dev_id}")
    else:
        logger.warning(f"Rastreador J16W-J16 não está conectado. Impossível enviar comando. device_id={dev_id}")
