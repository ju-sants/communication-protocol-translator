...
def handle_connection(conn: socket.socket, addr):
    """
    Lida com uma única conexão de cliente J16W, gerenciando o estado da sessão.
    """
    logger.info(f"Nova conexão J16W recebida endereco={addr}", tracker_id="SERVIDOR")
    buffer = b''
    dev_id_session = None
    ...
    try:
        while True:
            with logger.contextualize(tracker_id=dev_id_session):
                data = conn.recv(1024)
                if not data:
                    logger.info(f"Conexão J16W fechada pelo cliente endereco={addr}, device_id={dev_id_session}")
                    break
                    ...
                    if buffer.startswith(b'\x78\x78') or buffer.startswith(b"\x79\x79"):
                        ...
                        if len(buffer) >= full_packet_size:
                            ...
                            if not raw_packet.endswith(b'\x0d\x0a'):
                                logger.warning(f"Pacote J16W com stop bits inválidos, descartando. pacote={raw_packet.hex()}")
                                continue
                            
                            # Corpo do pacote que vai para o processador: [Length(1) + Proto(1) + Conteúdo + Serial(2) + CRC(2)]
                            packet_body = raw_packet[2:-2]
                            logger.info(f"Recebido pacote J16W: {packet_body.hex()}")
        ...
        logger.warning(f"Conexão J16W fechada abruptamente endereco={addr}, device_id={dev_id_session}", tracker_id="SERVIDOR")
    except Exception:
        logger.exception(f"Erro fatal na conexão J16W endereco={addr}, device_id={dev_id_session}", tracker_id="SERVIDOR")
    finally:
        logger.debug(f"[DIAGNOSTIC] Entering finally block for J16W handler (addr={addr}, dev_id={dev_id_session}", tracker_id="SERVIDOR")
        if dev_id_session:
            with logger.contextualize(tracker_id=dev_id_session):
        ...
        logger.info(f"Fechando conexão e thread J16W endereco={addr}, device_id={dev_id_session}", tracker_id="SERVIDOR")
