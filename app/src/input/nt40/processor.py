...
def process_packet(dev_id_str: str | None, packet_body: bytes) -> tuple[bytes | None, str | None]:
    ...
    if protocol_number == 0x01: # Pacote de Login
        ...
        if dev_id_str:
            packet_data, alarm_from_location_packet_data, alert_packet_data = mapper.handle_location_packet(dev_id_str, serial_number, content_body, protocol_number, packet_body.hex())
            if packet_data:
                send_to_main_server(dev_id_str, packet_data, serial_number, packet_body.hex(), original_protocol="NT40")
            if alarm_from_location_packet_data:
            ...
            if alert_packet_data and alert_packet_data.get("universal_alert_id"):
                send_to_main_server(dev_id_str, alert_packet_data, serial_number, packet_body.hex(), original_protocol="NT40", type="alert")
        else:
        ...
        if dev_id_str:
            if mapper.handle_heartbeat_packet(dev_id_str, serial_number, content_body, packet_body.hex()):
        ...
        if dev_id_str:
            alarm_packet = mapper.handle_alarm_packet(dev_id_str, serial_number, content_body, packet_body.hex())
            if alarm_packet:
        ...
        if dev_id_str:
            reply_packet = mapper.handle_reply_command_packet(dev_id_str, serial_number, content_body, packet_body.hex())
            if reply_packet:
