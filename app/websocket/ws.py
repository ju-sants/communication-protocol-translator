...
class LogStreamer(WebSocket):
    ...
    def connected(self):
        logger.info(f"Cliente conectado: {self.address}", tracker_id="SERVIDOR - WEBSOCKET SERVER")
    ...
    def handle(self):
        if self.tracker_id is None:
            ...
            logger.info(f"Mensagem subsequente de {self.address} ignorada: {self.data}", tracker_id="SERVIDOR - WEBSOCKET SERVER")

    def handle_close(self):
        logger.info(f"Cliente desconectado: {self.address}", tracker_id="SERVIDOR - WEBSOCKET SERVER")
        
        if self.log_process:
            logger.info(f"Encerrando o processo journalctl para o cliente {self.address}...", tracker_id="SERVIDOR - WEBSOCKET SERVER")
            self.log_process.terminate()
            self.log_process.wait()
            logger.info("Processo encerrado.", tracker_id="SERVIDOR - WEBSOCKET SERVER")
    ...
    def _stream_logs(self):
        ...
        try:
            ...
            if return_code:
                ...
                logger.info(f"Subprocesso journalctl terminou com erro. código {return_code}: {stderr_output}", tracker_id="SERVIDOR - WEBSOCKET SERVER")

        except Exception as e:
            logger.info(f"Ocorreu um erro na thread de streaming para {self.address}: {e}", tracker_id="SERVIDOR - WEBSOCKET SERVER")
        finally:
